...
## Q : CSR과 SSR의 차이에 대해서 설명해주세요.

### CSR (Client-Side Rendering)

1. **정의**:
- CSR은 클라이언트 측에서 웹 페이지를 렌더링하는 방식이다. 초기 HTML 파일은 최소한의 콘텐츠만을 포함하며, 자바스크립트를 통해 동적으로 페이지를 구성한다.
2. **동작 원리**:
- 초기 로드 시, 브라우저는 기본 HTML과 자바스크립트 파일을 다운로드한다. 자바스크립트가 실행되면서 필요한 데이터를 서버에서 받아와 화면을 동적으로 구성한다. 이후 페이지 간 이동은 전체 페이지를 다시 로드하지 않고, 필요한 데이터만을 갱신하여 빠르게 콘텐츠를 표시한다.

### 주요 특징

1. **초기 로드 시간**:
- 초기 로드 시간은 상대적으로 길다. 초기 HTML이 최소한의 콘텐츠만 포함하기 때문에, 자바스크립트 파일과 데이터를 모두 받아와 실행해야 전체 페이지가 완성된다.
2. **사용자 경험**:
- 첫 로드 이후, 페이지 간 전환이 빠르고 부드럽다. 페이지 전체를 다시 로드하지 않고 필요한 부분만 갱신하기 때문이다.
3. **SEO (Search Engine Optimization)**:
- SEO가 어렵다. 초기 HTML이 최소한의 콘텐츠만 포함하므로, 검색 엔진이 크롤링하기 어렵다. 이를 해결하기 위해 서버 측에서 초기 HTML을 렌더링해주는 프레임워크나 라이브러리를 사용할 수 있다.

### SSR (Server-Side Rendering)

1. **정의**:
- SSR은 서버 측에서 웹 페이지를 렌더링하는 방식이다. 서버에서 모든 HTML을 생성하여 클라이언트에 전달한다. 브라우저는 이를 받아 화면에 표시한다.
2. **동작 원리**:
- 서버는 클라이언트의 요청을 받아 필요한 데이터를 조회하고, 이를 바탕으로 완성된 HTML 파일을 생성하여 응답한다. 브라우저는 이 HTML 파일을 받아 즉시 화면에 표시할 수 있다. 자바스크립트가 추가적으로 실행되면서 인터랙티브한 요소를 활성화한다.

### 주요 특징

1. **초기 로드 시간**:
- 초기 로드 시간이 빠르다. 서버에서 완성된 HTML을 보내기 때문에 브라우저는 이를 즉시 렌더링할 수 있다.
2. **사용자 경험**:
- 초기 로드 이후의 사용자 경험은 CSR보다 느릴 수 있다. 페이지 간 전환 시, 서버에 새로운 요청을 보내고 전체 HTML을 다시 받아와야 하기 때문이다.
3. **SEO (Search Engine Optimization)**:
- SEO에 유리하다. 서버에서 완성된 HTML을 제공하므로, 검색 엔진이 페이지의 콘텐츠를 쉽게 크롤링할 수 있다.

### CSR과 SSR의 비교

1. **초기 로드 시간**:
- CSR은 자바스크립트와 데이터를 모두 받아와 실행해야 하므로 초기 로드 시간이 길다.
- SSR은 서버에서 완성된 HTML을 제공하므로 초기 로드 시간이 빠르다.
2. **페이지 전환**:
- CSR은 페이지 전환이 빠르고 부드럽다.
- SSR은 페이지 전환 시마다 서버에 요청을 보내므로 느릴 수 있다.
3. **SEO**:
- CSR은 초기 HTML이 빈약하여 SEO에 불리하다.
- SSR은 완성된 HTML을 제공하므로 SEO에 유리하다.

CSR과 SSR은 각기 다른 장단점을 가지고 있으며, 프로젝트의 특성과 요구사항에 따라 적절한 방식을 선택하는 것이 중요하다. SPA (Single Page Application)에서 CSR을 주로 사용하고, SEO가 중요한 페이지나 초기 로드 성능이 중요한 경우 SSR을 고려할 수 있다.