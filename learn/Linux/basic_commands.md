...
- `nohup` : 명령어를 로그아웃 후에도 계속 실행되도록 함
- `&` : 명령어를 백그라운드에서 실행

예시

```bash
nohup long_running_script.sh & # long_running_script.sh를 백그라운드에서 계속 실행
```
...
- `jobs` : 현재 쉘 세션에서 실행 중인 백그라운드 작업 목록을 표시

예시

```bash
jobs # 현재 쉘 세션의 모든 백그라운드 작업 표시
```

## fg : 백그라운드 작업을 포어그라운드로 가져오기

> 기본 형식: fg <작업번호>

설명

- `fg` : 백그라운드에서 실행 중인 작업을 포어그라운드로 전환

예시

```bash
fg %1 # 작업 번호 1번을 포어그라운드로 전환
```

## bg : 포어그라운드 작업을 백그라운드로 보내기

> 기본 형식: bg <작업번호>

설명

- `bg` : 포어그라운드에서 일시 중지된 작업을 백그라운드에서 실행

예시

```bash
bg %1 # 작업 번호 1번을 백그라운드에서 계속 실행
```
