...
- 기본 선택자에 이어서 결합자, 속성 선택자, 의사 클래스/의사 요소까지 살펴봤다.
다양한 선택자를 적절히 활용하면 HTML 구조가 복잡해지더라도 정확하고 효율적인 스타일링이 가능해진다.
- 솔직히 개발하면서 이렇게 복잡한 선택자를 쓰는 일은 없다. 요즘 워낙 라이브러리도 잘 나와 있고, 여하튼
선택자의 우선순위를 고려하면서, 불필요한 겹침 없이 깔끔하게 작성하는 습관을 들이는 것이 좋다.