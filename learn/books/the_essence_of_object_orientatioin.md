...
**부록 A는 객체지향 설계에서 추상화를 효과적으로 활용하기 위한 다양한 기법들을 상세히 설명했다. 추상화는 복잡한 시스템을 단순화하고, 핵심 개념을 강조함으로써 유지보수성과 확장성을 높이는 데 중요한 역할을 한다.**

## 추상화 기법

- 추상화 기법은 시스템의 복잡성을 줄이고, 중요한 개념을 강조하여 설계를 단순화하는 방법이다. 이를 통해 개발자는 시스템의 핵심 요소에 집중할 수 있으며, 불필요한 세부사항을 제거할 수 있다.

### 3가지 기법
1. 분류와 인스턴스화 : `분류`는 객체의 구체적인 세부 사항을 숨기고 인스턴스 간에 공유하는 공통적인 특성을 기반으로 범주를 형성하는 과정이다. 분류의 역은 범주로부터 객체를 생성하는 `인스턴스화` 과정이다.
2. 일반화와 특수화 : `일반화`는 범주 사이의 차이을 숨기고 범주 간에 공유하는 공통적인 특성을 강조한다. 일반화의 역을 `특수화` 라고 한다.
3. 집합과 분해 : `집합`은 부분과 관련된 세부 사항을 숨기고 부분을 사용해서 전체를 형성하는 과정을 가리킨다. 집합의 반대 과정은 전체를 부분으로 분리하는 `분해` 과정이다.

## 분류와 인스턴스화

- **분류(Classification)**: 객체들을 공통된 특성과 행동을 기준으로 그룹화하는 과정이다. 분류를 통해 유사한 객체들을 하나의 클래스나 인터페이스로 묶을 수 있다.
- **인스턴스화(Instantiation)**: 분류된 클래스를 기반으로 실제 객체를 생성하는 과정이다. 인스턴스화된 객체는 클래스의 특성과 행동을 상속받아 구체적인 기능을 수행한다.

### 개념과 범주

- **개념(Concept)**: 시스템 내에서 중요한 역할을 하는 추상적인 아이디어나 엔티티를 의미한다. 예를 들어, 명령(order), 고객(Customer), 자동차(Car), 택시(taxi), 트럭(truck). 버스(bus) 등이 개념에 해당한다.
  - 객체지향에서 개념을 가리키는 표준 언어는 `타입`이다.
- **범주(Category)**: 개념을 더 넓은 범위로 묶어주는 그룹이다. 범주는 개념들을 유사한 특성에 따라 분류하여 체계적으로 관리할 수 있게 한다. 예를 들어서 자동차(Car)

### 타입(Type)

- 타입은 객체의 행동과 역할을 정의하는 추상적인 분류이다. 타입을 통해 객체들은 특정한 메시지를 수신할 수 있으며, 이는 객체지향 설계에서 다형성을 구현하는 기반이 된다.
  - 심볼, 내연, 외연 (3장 참고)

### 외연과 집합

- **외연(Extension)**: 특정 타입에 속하는 모든 객체들의 집합을 의미한다. 예를 들어, `Customer` 타입의 외연은 모든 고객 객체들을 포함한다.
- **집합(Set)**: 외연과 유사하게 특정 타입의 객체들을 모아놓은 그룹이다. 집합을 통해 객체들을 체계적으로 관리하고 접근할 수 있다.
- 단일 분류(single classification) : 한 객체가 하나의 타입에만 속하는 것
  - 대부분 객체지향은 프로그래밍 언어는 단일 분류만을 지원
- 다중 분류(multiple classification) : 동시에 여러 타입에 속하는 것
  - 다중 분류는 다중 상속과 다르다!!

### 클래스(Class)

클래스는 객체지향 설계의 기본 단위로, 객체의 속성과 행동을 정의하는 청사진이다. 클래스는 타입의 구체적인 구현을 제공하며, 이를 통해 객체는 특정한 역할과 책임을 수행할 수 있다.

## 추상화와 특수화

- **추상화(Abstractization)**: 공통된 특성과 행동을 추출하여 추상 클래스를 만드는 과정이다. 이를 통해 코드의 재사용성과 유연성을 높일 수 있다.
- **특수화(Specialization)**: 일반적인 클래스를 기반으로 특정한 특성을 추가하여 구체적인 클래스를 만드는 과정이다. 특수화를 통해 다양한 객체들이 공통된 인터페이스를 유지하면서도 고유한 기능을 수행할 수 있다.

### 범주의 계층

범주의 계층은 일반적인 타입에서 특수한 타입으로의 상속 구조를 의미한다. 이를 통해 상위 타입은 공통된 특성과 행동을 정의하고, 하위 타입은 이를 확장하여 구체적인 기능을 추가할 수 있다.

### 서브타입(Subtype)

- 서브타입은 상위 타입을 확장하여 특정한 특성과 행동을 추가한 타입이다. 서브타입은 상위 타입과 호환되며, 다형성을 통해 유연한 객체 지향 설계를 가능하게 한다.
  - 일반적인 상위 타입은 슈퍼타입(supertype)
  - 100 % 규칙 : 슈퍼타입의 정의가 100% 서브타입에 적용돼야만 한다.
  - Is-a 규칙 : 서브타입의 모든 인스턴스는 슈퍼타입 집합에 포함돼야 한다.

### 상속(Inheritance)

- 상속은 기존 클래스의 특성과 행동을 새로운 클래스가 물려받는 기법이다. 이를 통해 코드의 재사용성을 높이고, 계층적인 타입 구조를 형성할 수 있다.
- 서브타이핑(subtyping) = 인터페이스 상속(interface inheritance)
  - LSP 만족
- 서브클래싱(subclassing) = 구현 상속(implementation inheritance)
  - LSP 충족 못함, 슈페클래스를 대체 못함

### 위임(delegation)
- 자식 객체는 수신된 메시지를 처리하지 못하면, 부모 객체에게 위임한다. 그 부모도 처리 못하면 그 부모의 부모에게 위임(실행될 때까지 계속)
 
### 집합과 분해

- **집합(Set)**: 특정 타입의 객체들을 모아놓은 그룹으로, 집합을 통해 객체들을 체계적으로 관리할 수 있다.
  - 불필요한 세부 사항은 무시 가능하다.(캡슐화)
- **분해(Decomposition)**: 복잡한 시스템을 더 작은 단위로 나누어 설계하는 과정이다. 분해를 통해 시스템의 각 부분을 독립적으로 관리하고 설계할 수 있다.

## 계층적인 복잡성(Hierarchical Complexity)

- 시스템의 복잡성을 계층적으로 관리하여, 상위 계층이 하위 계층을 제어하고 조정하도록 설계하는 방법이다. 이를 통해 시스템의 구조를 명확히 하고, 변경에 유연하게 대응할 수 있다.

## 합성 관계(Composition)

- 합성 관계는 객체들이 다른 객체들을 포함하여 복잡한 구조를 형성하는 관계이다. 
  - 부분을 전체 안에 캡슐화해 인지 과부하를 방지한다.
- 합성을 통해 객체들은 서로 협력하여 더 복잡한 기능을 수행할 수 있으며, 유연한 설계를 가능하게 한다.

## 패키지(Package)
> 모듈(module) 과 동일

- 패키지는 관련된 클래스와 인터페이스를 그룹화하여 관리하는 단위이다.
- 패키지를 통해 시스템의 구조를 체계적으로 조직하고, 네임스페이스 충돌을 방지할 수 있다.