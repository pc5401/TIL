...
## 응집도 (Cohesion)
#### 정의
응집도는 모듈 내의 요소들이 얼마나 밀접하게 관련되어 있는지를 나타내는 개념이다. 높은 응집도를 가진 모듈은 내부 요소들이 같은 기능을 수행하는 데 집중되어 있으며, 이는 모듈의 이해와 유지보수를 용이하게 한다.

#### 특징
- 모듈 내부의 요소들이 같은 목적을 달성하기 위해 협력함.
- 응집도가 높을수록 모듈의 재사용성과 유지보수성이 높아짐.
- 응집도는 일반적으로 높은 응집도에서 낮은 응집도로 나뉨.

#### 종류 (높은 응집도에서 낮은 응집도 순)
...
2. 순차적 응집도 (Sequential Cohesion)
   - 한 기능의 출력이 다른 기능의 입력으로 사용되는 경우.
   - 예: 데이터를 읽고 처리한 후 결과를 출력하는 모듈.

3. 교차적 응집도 (Communicational Cohesion)
   - 같은 데이터를 사용하거나 같은 파일을 다루는 기능들이 모여 있는 경우.
   - 예: 동일한 데이터베이스를 접근하는 모듈들.

4. 절차적 응집도 (Procedural Cohesion)
   - 모듈 내의 기능들이 특정 순서로 수행되어야 하는 경우.
   - 예: 특정 작업을 완료하기 위해 순차적으로 실행되는 함수들.

5. 시간적 응집도 (Temporal Cohesion)
   - 특정 시간에 실행되어야 하는 기능들이 모여 있는 경우.
   - 예: 초기화 루틴, 종료 루틴.

6. 논리적 응집도 (Logical Cohesion)
   - 비슷한 성격의 기능들이 모여 있는 경우.
  - 예: 다양한 오류 처리 기능을 포함하는 모듈.

7. 우연적 응집도 (Coincidental Cohesion)
   - 서로 관련 없는 기능들이 단순히 한 모듈에 모여 있는 경우.
   - 가장 낮은 수준의 응집도.
   - 예: 임의의 함수들이 모여 있는 유틸리티 모듈.

## 결합도 (Coupling)
#### 정의
결합도는 하나의 모듈이 다른 모듈과 얼마나 의존적인지를 나타내는 개념이다. 결합도가 낮을수록 모듈 간의 의존성이 적어지고, 소프트웨어 시스템의 유지보수성과 확장성이 높아진다.

#### 특징
- 모듈 간의 의존성을 최소화하는 것이 목표.
- 결합도가 낮을수록 시스템의 유연성과 재사용성이 높아짐.
- 결합도는 일반적으로 높은 결합도에서 낮은 결합도로 나뉨.

#### 종류 (높은 결합도에서 낮은 결합도 순)
1. 내용 결합도 (Content Coupling)
   - 한 모듈이 직접 다른 모듈의 내부 기능이나 데이터를 참조하는 경우.
   - 가장 높은 수준의 결합도.
...
3. 외부 결합도 (External Coupling)
   - 모듈이 외부의 데이터 포맷, 통신 프로토콜 등을 공유하는 경우.
   - 예: 파일 형식, 데이터베이스 스키마.

4. 제어 결합도 (Control Coupling)
   - 한 모듈이 다른 모듈의 동작을 제어하기 위해 제어 신호를 전달하는 경우.
   - 예: 플래그 변수로 다른 모듈의 흐름을 제어.

5. 스탬프 결합도 (Stamp Coupling)
   - 모듈이 복잡한 데이터 구조를 공유하는 경우.
   - 예: 구조체나 객체를 파라미터로 전달.

6. 자료 결합도 (Data Coupling)
   - 모듈이 단순한 데이터를 주고받는 경우.
   - 가장 낮은 수준의 결합도.
   - 예: 기본 데이터 타입을 파라미터로 전달.