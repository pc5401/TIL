...
## 동적 함수 호출 (Dynamic Function Call)

JavaScript에서 동적 함수 호출은 함수 이름을 문자열로 다루고, 이를 사용하여 객체의 메소드를 런타임에 호출하는 기법입니다. 이 방법을 사용하면 함수 이름이 런타임에 결정될 때 유용합니다.

### 기본 개념

동적 함수 호출은 객체의 메소드를 동적으로 접근하고 호출하는 방법. 이를 통해 함수 이름과 인자를 유연하게 처리할 수 있다.

### 구문 및 예제

#### 기본 사용법

객체의 메소드를 동적으로 호출하려면 대괄호 표기법과 전개 연산자를 사용.
...
obj[methodName](...args); // Output: Hello, World!
```

### 동적 함수 호출의 구성 요소
1. 객체 프로퍼티 접근 (Object Property Access): 객체의 프로퍼티(메소드 포함)에 대괄호 표기법을 사용하여 동적으로 접근하는 방법.
   - 예: obj[methodName]은 obj.sayHello와 동일한 의미.
2. 전개 연산자 (Spread Operator): 배열을 함수 호출의 인자로 펼치는 방법.
   - 예: ...args는 배열 args의 요소를 개별 인자로 펼침.
...
// 동적 함수 호출
func[funcName](...argsList); // 출력: 1 2 3
```

### 장점
- 유연성: 함수 이름과 인자를 런타임에 결정할 수 있어 유연한 코드 작성이 가능합니다.
- 리플렉션: 객체의 메소드 이름을 문자열로 다루어 리플렉션과 유사한 기능을 구현할 수 있습니다.

### 사용 사례
- 플러그인 시스템: 다양한 플러그인을 동적으로 호출할 때 유용합니다.
- 이벤트 핸들러: 이벤트 이름에 따라 적절한 핸들러를 호출할 때 사용됩니다.
- API 요청: API 엔드포인트 이름을 동적으로 설정하고 호출할 때 유용합니다.

#### 주의
- 안전성: 동적으로 호출할 함수 이름이 올바른지 확인하지 않으면 런타임 오류가 발생할 수 있습니다.
- 성능: 과도한 동적 호출은 성능에 영향을 미칠 수 있습니다. 필요한 경우 캐싱 등의 최적화 기법을 사용할 수 있습니다.